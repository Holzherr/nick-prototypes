...
import { themeById } from '@/features/personalise/themes';

/** Faint shapes drifting up behind every screen. Positions are fixed per index so renders never reshuffle. */
export const FloatingHearts = ({ count = 10 }: { count?: number }) => {
  const emoji = themeById(useTheme()).drift;
  return (
