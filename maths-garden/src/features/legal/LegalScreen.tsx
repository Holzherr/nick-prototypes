...
export function LegalScreen({ document: doc }: { document: LegalDocument }) {
  const t = useT();
  const other = doc.slug === 'terms' ? { href: '#/privacy', label: t('footer.privacy') } : { href: '#/terms', label: t('footer.terms') };
  return (
    <main className="mx-auto max-w-[760px] px-5" lang="en" dir="ltr">
      ...
      <h1 className="mt-6 text-[clamp(30px,5vw,48px)] font-bold leading-tight text-raspberry">{doc.title}</h1>
      ...
      <article>
        ...
        {doc.sections.map((section) => (
          <section key={section.heading}>
            <h2 className="mt-10 text-[clamp(22px,3vw,28px)] font-bold text-raspberry">{section.heading}</h2>
