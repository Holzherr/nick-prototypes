...
import { between, choices, makeQuestion, makeRound, questionKey } from './questions';
...
describe('makeQuestion', () => {
  it('keeps every game inside its level range', () => {
    const rng = seeded(3);
    for (const game of GAMES) {
      for (const max of game.levels) {
        for (let i = 0; i < 200; i++) {
          const q = makeQuestion(game.id, max, rng);
          switch (q.game) {
            case 'more':
              expect(q.left).not.toBe(q.right);
              expect(Math.max(q.left, q.right)).toBeLessThanOrEqual(max);
              expect(Math.min(q.left, q.right)).toBeGreaterThanOrEqual(1);
              expect(q.answer).toBe(q.left > q.right ? 'left' : 'right');
              break;
            case 'add':
              expect(q.base).toBeGreaterThanOrEqual(1);
              expect(q.extra).toBeGreaterThanOrEqual(1);
              expect(q.extra).toBeLessThanOrEqual(3);
...
describe('makeRound', () => {
  it('never asks the same question twice in a row', () => {
    const rng = seeded(4);
    for (let i = 0; i < 100; i++) {
      const round = makeRound('peek', 3, rng);
