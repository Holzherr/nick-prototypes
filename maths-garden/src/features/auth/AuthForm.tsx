  ...
  return (
    <div className="relative z-10 flex min-h-dvh items-center justify-center px-5 py-10">
      <Card className="w-full max-w-[440px]">
        ...
        <p className="mt-3 text-center text-grape/70">
          Grown-ups sign in here. Your child plays without needing to, and this
        ...
        <form onSubmit={submit} className="mt-6 flex flex-col gap-3">
          ...
          {padded && (
            <p className="text-sm font-medium text-clay">
              There’s a space at the{" "}
              {password !== password.trimStart() ? "start" : "end"} — autofill
              does that, and it will be rejected.{" "}
              <button
                type="button"
                className="font-semibold text-raspberry underline"
          ...
          {mode === "sign-up" && (
            ...
            <label
              htmlFor="agree-terms"
              className="flex items-start gap-3 text-sm leading-snug text-grape/80"
            >
              <input
                id="agree-terms"
                type="checkbox"
          ...
          {error && (
            <p className="text-sm font-medium text-raspberry">{error}</p>
          )}
          {notice && (
            <p className="text-sm font-medium text-leaf-deep">{notice}</p>
          )}
        ...
        {guestProfiles.length === 0 && (
          <>
            ...
            <p className="mt-2 text-center text-xs text-grape/55">
              Play now without an account. Progress stays on this device.
            </p>
          </>
        )}
      </Card>
