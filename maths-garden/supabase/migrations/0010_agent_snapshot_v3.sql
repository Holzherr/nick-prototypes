-- agent_snapshot v3: feedback rows carry who is speaking and what sort of thing it is.
--
-- 0007 gave `maths_feedback` a `reporter` and a `kind`; the form has sent them since, and 0009 never read
-- them, so the Analyst saw every message as one voice. One change from 0009: the `feedback` CTE also
-- selects `f.reporter` and `f.kind`. Everything else — signature, role, grants, hashing, the other five
-- sections — is copied unchanged. Still no reply address and still no session id.
--
-- Runs after 0007: the two columns must exist. A `language sql` body is checked when the function is
-- created, so on a database without 0007 this file fails on `f.reporter` and leaves 0009's function in
-- place; apply 0007 first. Apply as 0008 and 0009 were, by hand in the SQL editor.
...
feedback as (
  -- Message, path, locale, at, reporter and kind: never the optional reply address someone may have left,
  -- and never the session id that would tie a message back to a visit.
  select f.message, f.path, f.locale, f.at, f.reporter, f.kind
