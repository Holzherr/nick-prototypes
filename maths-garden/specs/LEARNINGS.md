...
- 2026-09-20 builder: MG-003 has no file to change: Site URL and Confirm-email-off already sit in supabase/config.toml (Nick applies them with `npx supabase config push --project-ref gzdfoptvdocauvgxltjk` after `config diff`), and the Google client id/secret is a secret on Nick's MacBook that cannot enter the public repo; a dashboard-only infra item needs Nick or a machine with the CLI signed in, not a Builder.
