...
const test = async () => {
  ...
  const fixtures = [
    { name: 'Hotaru no haka', year: 1988, type: 'movie', aliases: ['Grave of the Fireflies'], why: 'test' },
  ...
  it('a failed row with no alias is not selected', () => {
    const rows = [
      { name: 'Hotaru no haka', year: 1988, type: 'movie', status: 'failed' },
      { name: 'The Lantern Ledger', year: 1993, type: 'movie', status: 'failed' },
    ];
    assert.deepEqual(aliasedRows(rows, keys).map((row) => row.name), ['Hotaru no haka']);
  });
  it('a row is matched on name, year and type together', () => {
    const rows = [
      { name: 'Hotaru no haka', year: 1989, type: 'movie' },
      { name: 'Hotaru no haka', year: 1988, type: 'series' },
  ...
  it('matching folds case, accents and punctuation the way the queue does', () => {
    assert.deepEqual(aliasedRows([{ name: 'HOTARU NO HAKA!', year: 1988, type: 'movie' }], keys).length, 1);
  ...
  it('check accepts the shipped file', () => {
    ...
    assert.deepEqual(aliasesFor({ name: 'Hotaru no haka', year: 1988, type: 'movie' }), ['Grave of the Fireflies']);
