# Pinpoint browser extension (MVP scaffold)

Freezes the current tab and opens it in the Pinpoint annotator (the hosted website) via the
URL fragment — reusing the viewer's `#<lz>` hydrate path.

## Load it (Chrome/Edge, dev)
1. `chrome://extensions` → enable **Developer mode**.
2. **Load unpacked** → select this `extension/` folder.
3. Click the Pinpoint toolbar icon on any page → **Capture & annotate this page**.
...
`.html` then "Upload a snapshot", or the **CLI** (`pinpoint annotate <url>`). Roadmap: vendor
