...
- `spaces-lab.html` — **lab** (unlisted): the **Spaces / entities** concept (Personal · Family · Business), two ways: **A) the Venn** — overlapping Spaces, modules *shared by reference* in the overlaps, "you" at the centre; **B) the "Me" lens** — three Spaces rolling a thread each into one personal dashboard. Communicates `00-echo-spec.md §7`.
...
- `family-hub.html` — **consumer product page** for the **Echo Family Hub** (the pre-built wall tablet). Warm, plain-language pitch ("your home, on the wall"), a 3-step "arrives ready" strip, a consumer FAQ, and two-tier pricing: **Done-for-you £279 one-time** (≈ $349/€329, built+installed+shipped) and a **free DIY guide** (bring your own tablet). Pricing assumes a fulfilment partner builds/ships; £279 ≈ 2× a ~£133 bundle COGS (50% gross margin). Membership is separate. Cost/margin breakdown is *not* on the page (public site) — see chat/commit history.
...
- `blog.html` — **Notes**: the blog index. Renders the post list from a `POSTS` array in the page (newest first) — no build step. Uses the standard nav/footer so it's reachable from every page and links back.
- `blog/` — individual posts, each a self-contained HTML file in the house style (nav links use `../`).
  - `blog/the-interface-is-the-moat.html` — first post: why we agree with **Benedict Evans** that foundation models are commoditising and value accrues *above* the model (memory · distribution · the interface that does the work), mapped onto how Echo is built.
  - `blog/_template.html` — copy to start a new post. **To publish:** `cp blog/_template.html blog/<slug>.html`, fill in title/meta/heading/body, then add one entry to `POSTS` at the top of `blog.html` (slug = filename without `.html`).
