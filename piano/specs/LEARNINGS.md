...
- 2026-09-25 builder: a sync `act()` does not drain microtasks, so a loop of presses in a test yields two upserts, the first and the last; drain with an async act before reading fetch calls. jsdom 30 has `crypto.randomUUID`.
